...
export default function DashboardAdminKab() {
  ...
  const [saranaWisatas, setSaranaWisatas] = useState<SaranaWisataUnggulan[]>(
    []
  );
  ...
  const fetchSarana = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const nonWisata = data.filter(
          (sarana: Sarana) => sarana.kategori !== "wisata"
        );
        const wisata = data.filter(
          (sarana: Sarana) => sarana.kategori === "wisata"
        );
        setSaranas(Array.isArray(nonWisata) ? nonWisata : []);
        setSaranaWisatas(Array.isArray(wisata) ? wisata : []);
      }
    } catch (error) {
  ...
  const getDashboardStats = (): DashboardStats => {
    ...
    return {
      ...
      totalSaranaWisata: saranaWisatas.length,
  ...
  const organizationData = [
    ...
    { name: "Sarana Wisata", value: stats.totalSaranaWisata, color: "#FF7300" },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-r from-indigo-500 to-indigo-600 text-white p-6 rounded-lg shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-indigo-100">Total Desa</p>
        ...
        <div className="bg-gradient-to-r from-indigo-500 to-indigo-600 text-white p-6 rounded-lg shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-indigo-100">Total User</p>
